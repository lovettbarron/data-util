...
# -*- encoding: utf-8
"""
Modified very, very slightly from Alex Chan's script to download all previously listened to podcasts. This will only download the ones you stared.

https://git.alexwlchan.net/?p=overcast-downloader;a=summary
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--recommended",
        const=True, default=True, type=str2bool, nargs='?',
        help="Only download podcast episodes you've starred i.e. recommended",
    )
    ...
    return {
        ...
        "recommended_only": bool(args.recommended)
    }
...
def get_episodes(xml_string):
    ...
    for feed in root.findall("./body/outline[@text='feeds']/outline[@type='rss']"):
        ...
        for episode_xml in feed.findall("./outline[@type='podcast-episode']"):
            ...
            try:
                episode["recommended_date"] = episode_xml.get(
                    "userRecommendedDate")
            except KeyError:
                episode["recommended_date"] = False
...
def skip_non_recommended(episode):
    print(episode)
    if(args["recommended_only"]):
        return not bool(episode["episode"]["recommended_date"])
    else:
        return False
...
def download_episode(episode, download_dir):
    ...
    if skip_non_recommended(episode=episode):
        return
    ...
    filename = get_filename(download_url=audio_url,
                            title=episode["episode"]["title"])

    # Within the download_dir, put the episodes for each podcast in the
    # same folder.
    podcast_dir = os.path.join(
        download_dir, _escape(episode["podcast"]["title"]))
    ...
    if os.path.exists(download_path):
        ...
        if cached_overcast_id != this_overcast_id:
            ...
            download_url(url=audio_url, path=download_path,
